...
- **OOS Validation Framework (Phase 1)**: New `qs-trader validate <plan>` command for static in-sample / out-of-sample strategy validation
  ...
  - Declarative pass/fail rule catalog: `oos_sharpe_min`, `oos_max_drawdown_max`, `is_to_oos_sharpe_decay_max`, `min_oos_trades`, `require_positive_oos_total_return`; `on_review_required` rules downgrade Pass → ReviewRequired without triggering Fail
  - Exit codes: `0` = Pass, `1` = Fail, `2` = ReviewRequired, `3` = Invalid (configuration error or child run failure), `4` = unhandled exception
  - Self-contained evidence pack written to `experiments/<exp>/validations/<validation_id>/`: `summary.json`, `effective_plan.yaml`, `report.html`, and `audit/` directory containing `environment.json`, `git.json`, `plan_sha256.txt`, `base_config_sha256.txt`, `holdout.json`
